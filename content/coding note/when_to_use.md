...
- In a room with 23 people, there is a 50%-50% chance of shared birthday.

##### Application on hash table
...
- It inserts `<item>` into `seq` so as to keep `seq` in ascending order.
...
- At the beginning avoids any need to traverse the list, but it requires to update the pointer to the head of the data structure.
