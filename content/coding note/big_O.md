...
summary: "The efficiency of algorithms"
date: 16-02-2023
categories:
  - "Coding"
tags:
...
- When an algorithm has the very bad time complexity only once in a while besides the time complexity that happens most of time.
  - *i.e.* When the array is fullfilled, the cost of insertion in `O(n)`. Otherwise, the cost is `O(1)`.
...
- When you see a problem where the number of elements gets halved each time, that will likely be a `O(log N)` runtime.
