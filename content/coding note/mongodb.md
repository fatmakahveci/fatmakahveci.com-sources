...
- It supports partial indexes limited to only those documents matching a criteria filter in order to increase efficiency and reduce the amount of storage space required.
...
- `"$not"` is a metaconditional so it can be applied on top of any other criteria.
...
- `$` operator returns the matching element. It  helpful when you do not know the index of the element.
...
- Metaoperations on a cursor includes
...
- `explain` command to see what MongoDB is doing when it executes the query.
  ...
  - `totalKeysExamined` := the number of keys within the index MongoDB walked through in order to generate the result set.
...
- If you have more than 32 MB of results MongoDB will just error out, refusing to sort that much data.
  - To avoid, you must create an index supporting the sort operation or use sort in conjunction with limit to reduce the results to below 32 MB.

### Compound indexes

- `db.users.createIndex({"age" : 1, "username" : 1})` makes an index on both age and username.
- Below holds for the most of the situations, when designing a compound index:
