...
- Django primarily composed of two components, which are projects and apps.
...
- 8080 is the default value, which is changable.
...
The order, in which you create _Templates_, _Views_, and _URLs_, doesn't matter since all works together closely.
...
- You can add database in your project by using `<app_name>/models.py` file.
...
- The default primary key is an `id` column, which consists of an integer that is incremented automatically.
...
- There are three ways to write a view in Django:
  - **1. Function-based view** takes an `HttpRequest` object and returns an `HttpResponse` object, both of which are defined in `django.http` module.
    - An instance of `HttpRequest` class
    ...
    - An instance of `HttpResponse` class
...
- You can create your own custom template tags.
- template tag: `{% tag %}`
