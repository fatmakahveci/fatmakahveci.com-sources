...
# l[2:5] = 100 # not allowed if the target is slice the right-hand side must be iterable even if it has just one item
...
2. `bytes` is a binary serialization format represented by a sequence of 8-bits integers that is fit for storing data on the filesystem or sending it across the Internet. That is why you can only create bytes containing ASCII literal characters.
...
- If you keep a big array an array is much more efficient because an array does not actually hold full-fledged float objects. It only hold the packed bytes representing their machine values, just like an array in the `C` language.
...
- `memoryview` refererences the object.
...
- It allows you to share memory between data structures like, PIL images, SQLlite databases, NumPy arrays without first copying.
...
# Create a list of memory view
print(list(mview[0:3]))

# Create a tuple of memory view
print(tuple(mview[0:3]))

# Update a value of memory view
mview[2] = 70
...
# a 4 is in the most significant byte of 2-byte unsigned integer is 1024.
numbers
```
...
- **If you only implement one of `__repr__` or `__str__`, choose the first because Python will call it as a fallback.**
...
- It helps debugging.
- They use less memory than regular object because they do store attributes in a per-instance `__dict__`.
...
- Additional attributes of named tuple in comparison to tuple:
  - Class attribute
    - `_fields`
