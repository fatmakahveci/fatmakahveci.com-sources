...
- It often hints the weak points, bottlenecks, and vulnarabilities.
...
- The vertex coloring problem seeks to assign a label to each vertex of a graph such that no edge links any two vertices of the same color.
...
1. Dig deep down into paths branching off of the starting point until it reaches a point where either there is no more edge forward or meet a previously visited vertex. We don't want to revisit.
...
- **Complete graph:** It is a family of graph of which each pair of vertices is connected to one edge. `The number of vertices = $\frac{[n(n-1)]}{2}$
...
- **Density:** It is defined as the degree to which network nodes are connected one to another. It can be used as a measure of how close a network is to complete.

- **Diameter:** The highest eccentricity of its nodes. It represents the maximum distance between nodes.

- **Directed graph:** It is a family of graph of which every edge is an arrow.
...
- **Homophily:** It is the tendency of individuals to connect with others who share the same attitudes and beliefs.

- **In-degree:** The number of arrow going in to a vertex.
...
- **Out-degree:** The number of arrow going out of a vertex.
...
- **Simple graph:** It is a family of graph that has no loop and has no multiple edges.
...
- It counts the connection between edge and vertices. ($v \times e$ matrix)
- If an edge is connected to a vertex, the vertex is said to be incident to and edge.
...
- Convert grid to a adjacency list/matrix.
...
- It requires an embedded graph, where each vertex is associated with the point on this world where they live.
...
- Every edge either points away from of towards the root node.
...
- It can be colored without conflicts while using only two colors.
- It can only have even edge length cycle.
...
- Only a small fraction of the possible vertex pairs actually have edges defined between them.
...
- It is often easier to work with an implicit graph and store the entire thing prior to analysis.
...
class Graph:
    ...
    def strongly_connected_component(self):
        departure = [] # order of departure is the departuring time
        visited = []
...
- There is an **ordering** on the nodes of the graph, for every directed edge $(u,v)$, vertex $u$ comes before $v$ in the ordering.
  - i.e. School class prerequisities, etc.
...
- A closed walk at least one edge in which no vertex except the terminal vertices appear more than once.
...
- A spanning tree is a subgraph of the undirected connected graph with the minimum possible number of edges.
  - The subgraph should contain each and every node of the original graph.
...
- Greedy algorithm
- It starts with the single source node and later explore all the adjacent nodes of the source node with all the connecting edges. While we are exploring the graphs, we will choose the edges with the minimum weight and those which cannot cause the cycles in the graph.
...
- The maximum spanning tree can be found by negating the edge weights of the input graph G and using a minimum spanning tree algorithm on the result. The spanning tree of G that has the most negative weight will define the maximum-weight tree in G.
  - i.e. Suppose an evil telephone company is contracted to connect a bunch of houses together, such that they will be paid a price proportional to the amount of wire they install. Naturally, they will seek to build the most expensive possible spanning tree.

##### Minimum bottleneck spanning tree

Sometimes we seek a spanning tree that minimizes the maximum edge weight over all possible trees. In fact, every minimum spanning tree has this property. The proof follows directly from the correctness of Kruskal’s algorithm.
...
- It provides all shortest path in a network with their lengths.
- **Runtime:** $O(n^2)$
...
6. If no temporary nodes left, STOP.
