...
- It is a special Python function that behaves like a generator and yields control back to the event loop when it blocks.
...
- When we create a `task`, we create an empty `future` and running the `coroutine`. Then, when the coroutine has completed with either an exception or a result, we set the result or exception of the future.
- Each task maintains a single stack, and its own execution pointer as well.
- A task object can be used to monitor the status of the underlying coroutine.

#### Running concurrently with tasks

- A task makes coroutine running concurrently.
- It is a wrapper around the coroutine that schedule a coroutine to run on the event loop as soon as possible so we can execute multiple tasks at roughly the same time.
...
- Cancelling a task will cause that task to a raise a `CancelledError` when we `await` it, which we can then handle as needed.
- `CancelledError` can only be thrown from an `await` statement. This means that if we call cancel on a task when it is executing plain Python code, that code will run until completion until we hit the next `await` statement (if one exists) and a `CancelledError` can be raised. Calling cancel won’t magically stop the task in its tracks; it will only stop the task if you’re currently at an `await` point or its next `await` point.
...
- It takes in a coroutine or task object, and a timeout specified in seconds. Then, it returns a coroutine that we can `await`.
- If the task exceeds the specified timeout, a `TimeoutException` will be raised. Once we have reached the timeout threshold, the task will automatically be canceled.
...
- It has not only support as a HTTP client, but also has functionality to create asyncio-ready web application servers as well.
...
- **Drawbacks:**
  ...
  - If one of our coroutines has an exception, it will be thrown when we await the failed task. This means that we won't be able to process any tasks that completed successfully because that one exception will halt our execution.
...
- `gather` gives an optional parameter `return_exceptions`, which can specify how we want to deal with exceptions from our awaitables.
  - `return_exceptions=False` is the default. Both a coroutine and `gather` will throw an exception when we `await` it. Even a coroutine is failed, our other coroutines will continue to run as long as we handle the exception, or the exception does not result in the event loop stopping and canceling the tasks.
  - `return_exceptions=True` `gather` will return any exceptions as part of the result list it returns when we await it. `gather` will not throw any exceptions itself, and we'll be able handle all exceptions as we wish.
...
- Handling exceptions:
  - 1. We can use `await` and let the exception thrown and wrap it in `try except` block to handle the exception.
...
- We want to handle exceptions to ensure responsiveness.
  - 1. If there is no exception, `FIRST_EXCEPTION` equals to `ALL_COMPLETED`.
...
- To react the result as soon as it completes.
...
- `wait` sets timeouts to specify how long we want for all awaitables to complete with `timeout` parameter. If we have exceed the timeout, it returns both done and pending tasks.
  - 1. **Coroutines are not cancelled:** We must explicitly loop over the tasks and cancel them.
  - 2. **Timeout errors are not raised:**  If the timeout occurs the wait returns all tasks done and all tasks that are still pending up to that point when the timeout occurred.
...
- Tasks in the pending set are not canceled and will continue to run despite the timeout.
...
- At any one time the event loop can only have one Task actually executing, whilst the other tasks in the loop are all paused.
...
- When an event loop is triggered by either
 ...
 waiting coroutines will run until complete or hit the next `await` statement.
...
- It will allow other code to run concurrently, while we’re paused waiting for a connection.
...
async def main():
  ...
  for signame in [ 'SIGINT', 'SIGTERM' ]:
    loop.add_signal_handler(getattr(signal, signame), shutdown) # listen the event, it can safely interact with the event loop
...
- `JSON` is a data format of which format is similar to key-value store in Python dictionary. It’s the _defacto_ interchange format for most REST APIs.
...
- A socket allows multiple users connect simultaneously, letting them send and receive messages concurrently.
...
- `BaseSelector` can be used to wait for I/O readiness notification on multiple file objects.
  ...
  - `select` waits until some registered file objects become ready, or the timeout expires.
...
- We can directly listen for signals with `add_signal_handler` method, which can safely interact with the event loop.
...
server_socket = socket.socket()
server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) # socket.SO_REUSEADDR, 1 := reuse the port number after stop and restart the application
server_socket.setblocking(False)

server_address = ('127.0.0.1', 8000) 
server_socket.bind(server_address) # clients will use 127.0.0.1 to send data to our server
server_socket.listen() # allows clients to connect our server socket
...
while True:
  ...
  for event, _ in events:
    ...
    event_socket = event.fileobj # get the socket for the event, which are stored in the fileobj field

    if event_socket == server_socket: # This is a connection attempt.
      connection, client_address = server_socket.accept() # wait for a connection by blocking until we get a communication # connection := another socket that we read data from and write data to our client # client_address := the address of client that connected
      connection.setblocking(False)
...
- It runs the simultaneous things at the same time.
- Each process runs on a different core.
- It is the best when we have CPU-intensive work.
...
- It uses `cooperative multitasking`, runs one at a time. It cooperates by announcing when they're ready to be switched out.
...
- It allows us to pause execution of a particular method when we have an I/O operation; we can run other code while waiting for our initial I/O to complete in the background.
...
- The steps to write an asyncio program is as follows:
...
- User-created threads in Python do not receive `KeyboardInterrupt` exceptions; only the main thread will receive them. To handle this:
  ...
  2. **Cancelling or interrupting a running thread in our own way**
...
- `run_in_executor` (asyncio event loop) takes in a callable alongside an executor (either a thread pool or process pool) and runs that callable inside the pool. It returns an awaitable, which can used in an `await` statement or pass into an API function such as `gather`.
...
- Multiprocessing supports the `shared memory objects` concept.
  - `Shared memory object` is a chunk of memory allocated that a set of separate process can access. Each process can read and write into that memory space as needed.
...
- Using it in a coroutine or a task by itself, it will block the entire event loop until the request finishes.
...
- Avoid race conditions with locks from the `threading` module, `Lock` implementation.
  - Call its `acquire` and `release` methods around critical sections of code or using it in `context manager`.
...
- Use multithreading to run multiple event loops at the same time by building a responsive HTTP stresstesting user interface in Tkinter.
...
- `asyncpg` creates a connection pool using `create_pool` coroutine, which we use instead of `connect`.
  - Parameters for number of connections are `min_size` and `max_size`.
...
- Cursors can go forward only. To go backward, we execute SQL to do so use `DECLARE ... SCROLL CURSOR`.
...
class HTTPGetClientProtocol(asyncio.Protocol): # extends asyncio.Protocol class
 ...
 def connection_made(self, transport: Transport):
  """ `Transport` calls it when underlying socket has successfully
...
class ServerState:
  ...
  async def _on_connect(self, writer: StreamWriter):
    ...
    users are connected and notify any other connected clients that a
  ...
  async def _echo(self, reader: StreamReader, writer: StreamWriter):
    """when a user disconnects, we notifyany other connected clients
...
- `open_connection` will create
  ...
  - `StreamReader` to read the reponse.
...
- `StreamReaderProtocol` connects instances of stream readers to protocols.
  - Using this, a command-line application will not be blocked by the event loop when waiting the user input.
...
- `asyncio.start_server(<host>, <port>, <client_connected_cb>)` creates servers without managing sockets.
  - `<host>` and `<port>` are the address that the server socket will listen for connections.
  - `<client_connected_cb>` is either a callback function or a coroutine that will run whenever a client connects to the server.
    - This callback takes in a `StreamReader` and `StreamWriter` as parameters that will let us read from and write to the client that connected to the server.
...
- Most objects in `asyncio` provide an optional loop parameter that lets you specify the specific `event loop` to run in. When this parameter is not provided, `asyncio` tries to get the currently running `event loop`, but if there is none, it creates a new one. (Parameters will be removed with the version 3.10.)
...
- Locks to prevent concurrency bugs and synchronize `coroutine`s
  - They can sometimes be needed when shared state could change during `await`.
