...
- It checks that a small piece of code, like a function or class, as expected in isolation of the rest of the system.
- It could break during a refactoring or an implementation change.

#### What makes a good unit test?

1. It should clearly explain what behavior they're testing.
...
- It checks all of the system under test in an environment as close to the end-user environment as possible.

### 4. Functional testing

- It checks a single bit of functionality of a system.
- It should only break if we are intentionally changing functionality of the system.
...
`pytest -k "asdict or defaults" --collect-only` # , where asdict and defaults are the words in function's name
...
- An assert is not mechanism for handling runtime errors.
...
- For fixtures with scope other than function (class, module, session).
...
- `Parameter`s are different from `function argument`s.
  ...
  - The fixture called as many times as the number of elements in the iterable of `params` argument, and the test function is called with values of fixtures the same number of times.
...
- It places the test fixture in a separate standalone helper method that it is accessed by multiple test methods.

##### 3. Implicit setup

- It places the test fixture in a setup method which is used to set up multiple test methods.
- Each test method has its own setup procedures and linking to an external test fixture.
...
- It is the Python's mock object library. (`pip install mock`)
...
- It is optional.
- It contains project-wide pytest configuration.
- There should be at most only one in your project.
- It can contain directives that change the behavior of pytest, such as setting up a list of options that will always be used.
