...
BaseException
 ...
 +-- Exception
  +-- `StopIteration` is raised by next() and iterator's __next__() to signal there are no further items produced by the iterator.
      ...
      +-- `ImportError` is raised when the import statement has troubles trying to load a module.
      ...
      |    +-- `KeyError` is raised when a mapping key is not found un the set of existing keys.
      ...
      |    +-- `BlockingIOError` is raised when an operation would block on an object set for non-blocking operation.
      |    +-- `ChildProcessError` is raised when an operation on a child process failed.
      |    +-- `ConnectionError`
      |    |    +-- `BrokenPipeError` is raised when trying to write on a pipe thile the other end has been closed, or trying to write on a socket which has been shutdown for writing.
      ...
      +-- `RuntimeError` is raised when an error detected that doesn't fall in any of the other categories.
      ...
      |         +-- `TabError` is raised for an inconsistent use of tabs and spaces.
