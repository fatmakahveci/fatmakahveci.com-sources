...
- It can be thought of as resumable functions. Whereas normal functions execute and return a single result at a time, generators return a sequence of multiple results lazily, pausing after each one until the next one is requested. To create a generator, use the `yield` keyword instead of `return` in a function.
...
- A genexp saves memory as yielding items one by one using the iterator.
- It returns an iterator.

```python
symbols = '#@%*@#$'
# if genexp is the single argument in a function call, no need for duplicated paranthesis.
tuple(ord(symbol) for symbol in symbols)

import array
# array constructor takes two arguments, so the paranthesis of genexp are mandatory.
