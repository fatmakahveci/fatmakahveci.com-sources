...
- A decorator is a function that takes another function as an argument, adds some functionality, and returns another function without altering the original source code.
...
- Decorators can serve to
  - shorten code
  - speed up code
  - change the way that code acts

## 1. Function decorator

- Function decorators are more common than class decorators.

- If there is more than one decorator for a function, order of the decorators is important.
...
- We can use a decorator to keep track of state.
...
- It takes `cls` parameter that points to the class, not the object instance.
- It can't modify object instance state.
- Class methods are called via the Class containing it, rather than from an instance.
- `cls` parameter allows `@classmethod` to easily instantiate the class.
...
- Register a function as handler for an event.
