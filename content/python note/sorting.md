...
3. **Sort both:** We can compare the smallest elements of the two sorted sets, and discard the smaller one if they are not identical. By repeating this idea recursively on the now smaller sets.
...
1. `reverse`
   - If it is`True`, descending order.
...
- It accepts any iterable object as argument including
  - immutable sequences
  - generators
...
- Heapsort uses binary heap to sort an array in `O(nlogn)`.
