...
- Objects created by `partial()` have three read-only attributes:
  - `partial.func` returns the name of parent function along with hexadecimal address.
  - `partial.args` returns the positional arguments provided in partial function.
  - `partial.keywords` returns the keyword arguments provided in partial function.
"""
...
function, use @functools.wraps decorator in wrapper function, 
...
saved into cache. As a result, next time we go to call the function
