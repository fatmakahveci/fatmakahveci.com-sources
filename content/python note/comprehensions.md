...
- In contrast to list and set comprehensions, it needs two expressions separated with a colon followed by the usual `for` and `if` clauses.
...
- Not all for loop can be written as a dictionary, but all dictionary comprehension can be written with a for loop.
