...
- A DataFrame can contain:
  - **Pandas DataFrame**
  - **Pandas Series** := a one-dimensional labeled array capable of holding any data type with axis labels or index. i.e. one column from a DataFrame.
...
# Filtering a Dataframe: Selecting based on its own properties
...
## It excludes column with zero entries
df.loc[:, df.all()]

## It excludes column with all zeros
df.loc[:, df.any()]

## It excludes column with a NaN
