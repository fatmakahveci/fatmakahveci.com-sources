...
- To convert `byte` object into a mutable `bytearray` object, use the built-in `bytearray()` function.
...
- bytes literals may also use an *r* prefix to disable processing of escape sequences.
...
- `id(<object>)` returns identity (unique integer) of an object.
...
- `<set_name>.clear()`
- `<set_name>.remove(<item>)` #  if a value that doesn’t exist in the set, it raises a KeyError exception.
