...
- Pickling is not to be confused with compression!
  ...
  - *Compression* is the process of encoding data with fewer bits, in order to save disk space.

## Marshal

- Serializing a data means converting it into a string of bytes and later reconstructing it from such a string.
- If the data is composed entirely of fundamental Python objects, the fastest way to serialize the data is by using marshal module.
...
- The JSON module and XML are a good choice if we want to interoperability among different languages.
- Types are only
  - `null`
  - `boolean`
  - `numeric`
